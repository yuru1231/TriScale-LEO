...
/**
 * SlantRangeM — Euclidean distance between two 3-D positions (m).
 */
inline double
SlantRangeM(const Vec3& a, const Vec3& b)
{
...
/**
 * ElevationDegFromSat — elevation angle (degrees) from user to satellite.
 */
inline double
ElevationDegFromSat(const Vec3& satPos, const Vec3& userPos)
{
    const double dx    = satPos.x - userPos.x;
    const double dy    = satPos.y - userPos.y;
    const double dz    = satPos.z - userPos.z;
...
ComputeFSPL_dB(double distanceM, double freqHz)
{
    ...
    return 20.0 * std::log10(4.0 * M_PI * distanceM * freqHz / c);
}
...
{
    // Simplified zenith-scaling model (ITU-R P.618-13):
    //   A(ε) ≈ A_zenith / sin(ε),  A_zenith ≈ 0.55 dB for 30 GHz / Tokyo / p=1%
    // Matches Python's itur output to within < 0.3 dB for elevation > 20°.
    const double A_zenith = 0.55;
    const double elevRad  = std::max(5.0, elevationDeg) * M_PI / 180.0;
    return std::min(A_zenith / std::sin(elevRad), 25.0);
}
...
{
    const double dist = SlantRangeM(userPos, satPos);
    const double fspl = ComputeFSPL_dB(dist, cfg.centerFreqHz);
    const double elev = ElevationDegFromSat(satPos, userPos);
    const double atm  = ComputeAtmosphericLoss_dB(elev);
    return fspl + atm;
}
