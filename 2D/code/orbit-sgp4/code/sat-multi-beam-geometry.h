 ...
 *
 * Compute 25 beam-centre ground positions for a 5×5 along-track/cross-track
 * elliptic grid inscribed in the maximum rectangle of the elliptical footprint.
 *
 * At elevation θ:
 *   Ellipse semi-major (along-track) : a = r_footprint / sin(θ)
 *   Ellipse semi-minor (cross-track) : b = r_footprint
 *   Max inscribed rect half-widths   : W_along = a/√2,  H_cross = b/√2
 *
 * Along-track direction is estimated from the satellite displacement
 * (satEnu − prevSatEnu).  Pass satEnu for prevSatEnu on the first tick
 * (fallback: East-aligned).
 *
 * Row-major indexing (row=cross, col=along), row/col ∈ {−2,−1,0,+1,+2}.
 * Centre beam: index (0+2)×5+(0+2) = 12.
 *
 ...
 * @param cfg         Simulation config (rEarthM, rFootprintM).
 * @return            Array of 25 Vec3 beam-centre positions on the ground (z=0, ENU).
 */
std::array<Vec3, 25> GetEllipticBeamCenters(const Vec3& satEnu,
