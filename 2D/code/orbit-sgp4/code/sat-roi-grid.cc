...
EllipticFootprint
ComputeEllipticFootprint(double elevDeg, double rFootprintM)
{
    EllipticFootprint ef;
    ef.elevDeg     = elevDeg;
    ef.bSemiMinorM = rFootprintM;

    const double elevRad   = std::max(elevDeg, 5.0) * M_PI / 180.0;
    ef.aSemiMajorM     = rFootprintM / std::sin(elevRad);
    ef.rectAlongHalfM  = ef.aSemiMajorM / std::sqrt(2.0);
    ef.rectCrossHalfM  = ef.bSemiMinorM / std::sqrt(2.0);
    return ef;
}
