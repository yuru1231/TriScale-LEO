...
struct SatScanState
{
    ...
    // Observation grid config — same RF/antenna params as cfg but with
    // nBeamsX=gridD so GetFootprintRadius() spans the full ROI (~48 km for gridD=5).
    // Decouples beam count (cfg.nBeams=1) from observation grid size (gridD×gridD).
    SimConfig            gridCfg{};            // used by GetEllipticBeamCenters for user positions
