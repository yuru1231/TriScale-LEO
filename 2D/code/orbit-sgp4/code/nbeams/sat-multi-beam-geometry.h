 ...
 * driven by SGP4 orbit CSVs produced by run_sgp4.py.  Historical versions
...
// ---------------------------------------------------------------------------
// Nadir-shadow helpers
// ---------------------------------------------------------------------------

/**
 * GetNadirFromTime
 *
 * Compute the satellite sub-satellite point (nadir) at simulation time T.
 * Replaces frame-index lookup: instead of specifying which arc frame
 * corresponds to 90°/55°/25° elevation, the caller supplies time T (seconds)
 * and receives the geographic position of the nadir directly.
 *
 ...
 * @param cfg      Simulation config (defines the pass centre lat/lon,
 *                 orbital parameters).
 ...
 *
 * Elevation angle (degrees) from the local-frame origin to a 3D position
 * in ENU frame (x=East, y=North, z=Up).
 *
 *   horiz = sqrt(x² + y²)
 *   elev  = atan2(z, horiz)   [returns 90° when horiz ≈ 0]
 *
 * Used in grid/dual mode to compute elevation from the satellite ENU Vec3.
 *
 ...
 *
 * The existing beam-centre / user-position functions produce ECEF offsets
 * from the footprint centre.  In grid mode all positions must share the ENU
 * frame used by OrbitPointToEnuVec3, so this rotation is applied to beam
 * centres before passing them to ComputeFrameResults.
 *
 ...
 * At low elevation the 19 beams stretch along the slant direction into
 * an ellipse.  This function uses ray-ground intersection in ENU to
 * project each beam direction onto the z = 0 ground plane:
 *   Step 1 — nadir unit vector  : nadirUnit = normalize(-satEnu)
 *   Step 2 — beam frame basis   : Gram-Schmidt xSat/ySat from East
 *   Step 3 — beam direction ENU : bx·xSat + by·ySat + bz·nadirUnit
 *   Step 4 — ray-ground hit     : t = −satEnu.z / beamDir.z → ground pos
 *
 ...
 *
 * Compute nBeamsX × nBeamsY beam-centre ground positions for an along-track/
 * cross-track elliptic grid inscribed in the maximum rectangle of the elliptic
 * footprint.
 *
 * At elevation ε (D2 — both axes elevation-dependent):
 *   Ellipse semi-major (along-track) : a = r_footprint / sin²(ε)
 *   Ellipse semi-minor (cross-track) : b = r_footprint / sin(ε)
 *   Max inscribed rect half-widths   : W_along = a/√2,  H_cross = b/√2
 *
 * Along-track direction is estimated from the satellite displacement
 * (satEnu − prevSatEnu).  Required only when nBeamsX > 1; for nBeamsX == 1
 * the along-track direction is irrelevant (all beams are at the centre column).
 *
 * Row-major indexing (row=cross-track, col=along-track).
 * Centre beam: index (nBeamsY/2) × nBeamsX + (nBeamsX/2).
 *
 * @param satEnu      Satellite ENU position (m) at current tick.
 * @param prevSatEnu  Satellite ENU position (m) at previous tick (for along-track).
 * @param cfg         Simulation config (nBeamsX, nBeamsY, rEarthM, rFootprintM).
 * @return            Vector of nBeams Vec3 beam-centre positions on the ground (z=0, ENU).
 */
std::vector<Vec3> GetEllipticBeamCenters(const Vec3& satEnu,
 ...
 *
 * ECEF and ECI share the same z-axis (Earth's rotation axis), so only an
 * x-y plane rotation by -GAST is needed:
 ...
 * spherical Earth model (radius = rEarthM).  Consistent with the rest of
 * the framework which uses spherical Earth throughout.
 *
 * lat = asin(z / r)
 * lon = atan2(y, x)
 * alt = r - rEarthM
 ...
 */
void EcefToGeodetic(const Vec3& ecefM, double rEarthM,
