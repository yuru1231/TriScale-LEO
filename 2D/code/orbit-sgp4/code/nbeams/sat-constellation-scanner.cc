...
ScanSnrCallback(SatScanState* state, double timeS)
{
    ...
    // Require prevSatEnu before computing the observation grid (gridD×gridD needs
    // along-track direction).  Run() pre-computes prevSatEnu at (windowStart − dtSnrS)
    // for all satellites where tPrev ≥ 0.  This guard covers the rare edge case where
    // the pass window starts before simulation t=0.
    if (!state->hasPrevSat)
    {
        state->prevSatEnu = satEnu;
        state->hasPrevSat = true;
        return;  // skip first tick — no along-track reference available yet
    }

    // Beam hopping channel matrix:
    //   tickCellPos — gridD×gridD observation grid (both beam targets and users).
    //   ComputeBeamHoppingMatrix steers the beam to each of the nCells positions in turn
    //   and records the SNR at all nCells cells → full H[beam][cell] matrix per timestep.
    //   Path losses are computed once per cell (not per beam) for efficiency.
    const auto tickCellPos = GetEllipticBeamCenters(
        satEnu, state->prevSatEnu, state->gridCfg);  // gridD×gridD observation grid

    state->prevSatEnu = satEnu;  // update after grid computation

    const auto matrix = ComputeBeamHoppingMatrix(
        satEnu, tickCellPos, state->cfg);  // nCells² entries

    const int nCells = static_cast<int>(tickCellPos.size());

    for (const auto& entry : matrix)
    {
        // Geographic position of the observation cell (for cross-satellite analysis)
        double cellLat, cellLon;
        GetLatLon(tickCellPos[entry.obsCell], state->cfg, cellLat, cellLon);
...
SatConstellationScanner::Run(const ConstellationScanConfig& config)
{
    ...
    // Observation grid: gridD×gridD cells for channel evaluation.
    // Decoupled from beam count (cfg.nBeams=1) — the single nadir beam illuminates
    // all grid cells; each cell's SNR depends on its distance from the beam centre.
    const int nCells = config.gridD * config.gridD;  // 25 for gridD=5

    // Grid config: same RF/antenna params as cfg but nBeamsX/Y = gridD so that
    // GetFootprintRadius() returns the ROI-spanning footprint (~48 km for gridD=5,
    // nAntennaX=32).  rFootprintM=0 forces auto-compute from nBeamsX=gridD.
    SimConfig gridCfg   = config.cfg;
    gridCfg.nBeamsX     = config.gridD;
    gridCfg.nBeamsY     = config.gridD;
    gridCfg.nBeams      = nCells;
    gridCfg.rFootprintM = 0.0;
    ...
    std::cout << "[Constellation] 1 beam (nadir)  observation grid="
              << config.gridD << "x" << config.gridD
              << "  n_cells=" << nCells
              << "  footprint_r=" << std::fixed << std::setprecision(1)
              << gridCfg.GetFootprintRadius() / 1e3 << " km"
              << "  roi_lat=" << config.roiLatDeg
    ...
    for (const auto& sat : m_tleReader->GetSatellites())
    {
        ...
        info.nCellsCovered = nCells;  // gridD×gridD; matrix rows = nCells² per timestep
    ...
    for (std::size_t idx = 0; idx < passes.size(); ++idx)
    {
        ...
        state->cfg            = config.cfg;     // 1-beam channel config
        state->gridCfg        = gridCfg;        // gridD×gridD observation grid config
        ...
        // Beam hopping channel matrix CSV format:
        //   beam_idx   — which cell the beam is steered to (0..nCells-1)
        //   cell_idx   — which cell's SNR is measured      (0..nCells-1)
        //   When beam_idx == cell_idx: beam is on-centre → peak beam_gain_dB.
        //   snr_dB: channel quality for BH scheduling (no inter-beam interference).
        state->csv << "time_s,beam_idx,cell_idx,cell_lat_deg,cell_lon_deg,"
                      "elevation_deg,path_loss_dB,beam_gain_dB,snr_dB\n";
    ...
    const std::vector<SatPassInfo>& passes) const
{
    ...
    out << "  \"n_beams\": "       << config.cfg.nBeams     << ",\n";
    out << "  \"grid_d\": "        << config.gridD           << ",\n";
    out << "  \"n_cells\": "       << config.gridD * config.gridD << ",\n";
