...
Output : results/figures/fig_gap_timeline.png
         results/figures/fig_link_budget.png
         results/figures/fig_mrc_vs_greedy.png

Usage
-----
cd 2D/code/orbit-sgp4/analysis
python fig_coverage.py [--out-dir ../results] [--snr-thresh 0.0]

# Or run from results/ directory:
cd 2D/code/orbit-sgp4/results
python ../analysis/fig_coverage.py
"""
...
def ticks_to_segments(ticks):
    ...
    for t in ticks[1:]:
        if t == p + 1: p = t
        else: segs.append((s, p, p - s + 1)); s = p = t
...
def fig_gap_timeline(greedy_gaps, mrc_gaps, t_min, t_max, snr_thresh, fig_dir):
    ...
    for ax, label, segs in zip(axes, labels, [greedy_segs, mrc_segs]):
        ax.barh(0, window, left=t_min, height=0.6, color="#4caf50", linewidth=0)
        for gs, ge, dur in segs:
            ax.barh(0, dur, left=gs, height=0.6, color="#f44336", linewidth=0)
            if dur >= 10:
                ax.text(gs + dur / 2, 0, f"{dur} s",
                        ha="center", va="center",
        ...
        ax.text(0.99, 0.88, f"Gap: {total_gap} s ({pct:.1f}%)",
                transform=ax.transAxes, ha="right", va="top",
                fontsize=9, color="#f44336" if total_gap > 0 else "#4caf50")

    axes[1].set_xlabel("Time (s)", fontsize=10)
    fig.suptitle(
        f"Service Coverage Timeline\n"
        f"Threshold = {snr_thresh} dB  |  Green = service,  Red = gap",
        fontsize=10,
    )
...
def fig_link_budget(snr_thresh, min_elev_deg, fig_dir):
    ...
    for e_deg in [5.0, crit, 10.0, 20.0, 45.0, 90.0]:
        snr_val = snr_at_elevation(e_deg)
        ax.annotate(f"{snr_val:.1f} dB", xy=(e_deg, snr_val),
    ...
    ax.set_xlim(0, 90)
    ax.set_title(
        "Link Budget: SNR vs Elevation — Iridium-NEXT LEO (780 km, Ka-band)\n"
...
def fig_mrc_vs_greedy(greedy_gaps, mrc_gaps, window_s, snr_thresh, fig_dir):
    ...
    bars = ax.bar(
        ...
        [greedy_total, mrc_total],
        color=["#ef9a9a", "#a5d6a7"],
        edgecolor="white", linewidth=1.5, width=0.45,
    )
    for bar, val, pct in zip(bars, [greedy_total, mrc_total],
                              [greedy_pct, mrc_pct]):
    ...
    if greedy_total > 0 and mrc_total == 0:
        ...
        impr = (greedy_total - mrc_total) / greedy_total * 100
        ax.annotate(f"−{greedy_total - mrc_total} s\n({impr:.0f}% reduction)",
                    xy=(1, mrc_total + 5),
                    xytext=(0.7, (greedy_total + mrc_total) / 2),
                    fontsize=10, color="#1b5e20", fontweight="bold",
                    arrowprops=dict(arrowstyle="->", color="#1b5e20", lw=1.5))
    ax.set_ylabel("Total Gap Duration (s)", fontsize=11)
    ax.set_ylim(0, max(greedy_total, 10) * 1.3 + 10)
    ax.set_title(
