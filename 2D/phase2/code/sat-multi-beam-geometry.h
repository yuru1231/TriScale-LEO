 ...
 *
 *   elev = atan2(z_sat, |x_sat|)   (y_sat always 0)
 *
 ...
 *
 * Returns the satellite position (Vec3) in local Cartesian frame for a
 * given simulation time T (seconds from pass start).
 *
 * Converts time to arc angle:
 *   eps(T) = eps_zero + (T / t_frame) * d_eps
 *   x_sat  = R * cos(eps)
 *   z_sat  = R * sin(eps) − r_earth
 *   y_sat  = 0
 *
 * Used by Phase 1.2.2 (arc mode) where the satellite moves through a full
 * arc over the fixed footprint centre.
 *
 * @param timeS  Simulation time (seconds).
 * @param cfg    Simulation configuration.
 * @return       Satellite position Vec3 in local frame (metres).
 */
Vec3 GetSatellitePositionAtTime(double timeS, const SimConfig& cfg);
 ...
 *
 * Derivation (2D arc model, satellite in x-z plane):
 *   eps(T) = eps_zero + T / t_frame * d_eps
 *   nadir_local = { r_earth * cos(eps), 0, r_earth * sin(eps) - r_earth }
 * The nadir_local Vec3 is fed into GetLatLon() with the pass-centre config
 * to obtain (latDeg, lonDeg).
 *
 ...
 * Unlike GetElevationAngleDeg (which assumes y=0, arc mode only), this
 * version handles arbitrary (x, y, z) positions in the ENU frame where
 * x=East, y=North, z=Up.
 *
 *   horiz = sqrt(x² + y²)
 *   elev  = atan2(z, horiz)   [returns 90° when horiz ≈ 0]
 *
 * Used in grid mode to compute elevation from the satellite ENU Vec3.
 *
