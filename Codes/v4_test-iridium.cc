...
std::string islsFilePath =
    ...
    std::cout << "GW nodes: " << NodeList::GetNNodes() << std::endl;
...
std::ifstream f(islsFilePath);
if (!f.is_open())
{
    std::cerr << "ERROR: cannot open ISL file: " << islsFilePath << std::endl;
    return 1;
}
f.close();
    ...
    std::cout << "EventCount: "
              << Simulator::GetEventCount() << std::endl;
