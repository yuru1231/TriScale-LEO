...
//
// v6 相較 v5 的核心變更：
//   將 GW-to-GW 路由能力內建至 IslRoutingManager。
//   v5 的 FtVisibilityFilter 提供 "per-slot 最佳傳輸路由" 的概念，
//   v6 將其提升為一等公民，直接在 routing manager 中預計算並追蹤
//   src GW → dst GW 的完整端到端路由（含 entry sat、ISL 路徑、exit sat）。
//
//   新增函式：
//     AddGateway / AddGwPair / SetGwElevationThreshold
//     PrecomputeGwRoutes     ← 預計算所有 slot 的 GW-to-GW 路由
//     GetGwRoute             ← 查詢特定 slot 的 GW-to-GW 最佳路由
//     GetGwVisibleSats       ← 查詢特定 slot 某 GW 的可見衛星集合
//     PrintGwRouteReport     ← 跨 slot 路由報告，含 <ROUTE CHANGED> 標記
//     ComputeElevationDeg    ← 仰角計算（從 FtVisibilityFilter 移入統一管理）
...
{
    // 在每個 slot 的模擬時間點排程 ApplyRoutingTable
    for (uint32_t k = 0; k < m_numTimeSlots; k++)
    {
...
IslRoutingManager::ApplyRoutingTable(uint32_t slotIndex)
{
    ...
    // 在非初始 slot，檢查 load cost 是否有顯著變化，若有則局部重算
    if (slotIndex > 0)
    {
        UpdateLoadCosts();
    ...
    // 將 routing table 寫入每顆衛星的 arbiter
...
IslRoutingManager::GetPositionsAt(Time tau) const
{
    ...
    for (uint32_t i = 0; i < m_numSatellites; i++)
    {
        ...
        // GetGeoPositionAt 為已允許修改的 SNS3 擴充函式
...
IslRoutingManager::BuildISLGraph(const std::vector<Vector>& pos) const
{
    ...
    for (uint32_t edgeIdx = 0; edgeIdx < m_islDefs.size(); edgeIdx++)
    {
        ...
        // 跳過已封鎖的 ISL（僅在 RunAvoidanceTest 期間有效）
...
{
    // 與 BuildISLGraph 相同，但代價加上 EMA 平滑後的佇列延遲（load cost）
    const double   C       = 3e8;
    const double   maxDist = m_islMaxDistanceKm * 1e3;
...
{
    // 當本 slot 有多條等代價路徑時，選擇在下一 slot 仍存在的 next-hop，
    // 降低路由切換頻率。
    struct PairHash
    {
    ...
    for (uint32_t src = 0; src < routes.size(); src++)
    {
        ...
        for (auto& [dest, indices] : destToIndices)
        {
            ...
            // 在 tied 中找到下一 slot 仍存在的 next-hop，保留它，其餘設為 INF
            for (size_t idx : tied)
            {
...
IslRoutingManager::RebuildIslSources(uint32_t slotIndex)
{
    ...
    // 供 RecomputeAffectedRoutes 判斷哪些 source 受 edge 代價變化影響。
    m_islSources.assign(m_islDefs.size(), std::unordered_set<uint32_t>());
...
IslRoutingManager::LoadISLDefs(const std::string& islsFilePath)
{
    ...
    while (std::getline(file, line))
    {
        ...
        // 記錄每顆衛星在自己鄰居清單中的 interface index
...
// TracePath: 依照預計算的 routing table 重建 src→…→dst 完整路徑。
// 不重跑 Dijkstra，直接追 next-hop entry。
...
IslRoutingManager::TracePath(uint32_t src, uint32_t dst, uint32_t slotIndex) const
{
    ...
    for (uint32_t hop = 0; hop < m_numSatellites; ++hop)
    {
        ...
        if (!found)
        {
            path.push_back(UINT32_MAX); // 無法到達
            break;
        }
    }
...
// PrintRouteReport: 針對每個 (src,dst) 衛星對，逐 slot 輸出完整路徑與代價。
// 路徑與前一 slot 不同時標記 <PATH CHANGED>。
void
...
// BlockISL / UnblockISL: 供 RunAvoidanceTest 暫時標記 ISL 不可用
void
...
// RunAvoidanceTest:
//   1. 取得 (testSrc→testDst) 基線路徑
//   2. 封鎖路徑上第一條 ISL
//   3. 離線重算 routing table
//   4. 驗證：(a) 封鎖 ISL 不在新路徑中，(b) routing table 無殘留舊 next-hop
//   5. 還原 routing table 與 ISL 封鎖狀態
void
                                     ...
                                     uint32_t slotIndex)
{
    ...
    // 驗證 (b)：routing table 中不應有殘留的舊 next-hop
...
// AddGateway: 新增一個 Gateway 定義（id 必須唯一）
void
...
// AddGwPair: 設定允許路由的 GW pair（雙向插入）
void
...
// SetGwElevationThreshold: 設定 GW 可見衛星的最低仰角（度），預設 5°
void
...
//     2. 對每個 contracted GW pair (A, B)：
//        ‧ 枚舉 (entry ∈ visible(A)) × (exit ∈ visible(B))
//        ‧ 選出 ISL cost 最小的 (entry, exit)
//        ‧ 用 TracePath(entry, exit, slot) 重建完整衛星路徑
//        ‧ 雙向都儲存（A→B 和 B→A 各自獨立計算最佳 entry/exit）
//
...
IslRoutingManager::PrecomputeGwRoutes()
{
    ...
    // 用來去重：僅對每個 unordered pair {A,B} 計算一次（正反向各算）
    ...
    for (uint32_t k = 0; k < m_numTimeSlots; ++k)
    {
        ...
        for (const auto& p : m_gwPairs)
        {
            ...
            const auto& srcSatsBA = dstSatsAB; // 反向：src=B, dst=A
            ...
            for (uint32_t entry : srcSatsAB)
            {
                ...
                {
                    // 若 entry == exit，同一顆衛星同時可見兩個 GW，ISL cost = 0
...
// ── PrintGwRouteReport ────────────────────────────────────────────────────
//
// 輸出所有 contracted GW pair 的跨 slot 路由報告：
//   正向（A→B）與反向（B→A）分別輸出。
//   欄位：slot | time(s) | GW_src→dst | entry_sat | ISL_path | exit_sat | isl_cost(s)
//   若路徑相較前一 slot 有變化，標記 <ROUTE CHANGED>。
void
IslRoutingManager::PrintGwRouteReport() const
{
    // 取 GW 顯示名稱（若 name 為空，使用 "GW<id>"）
