# sat2ut — SAT → UT0 (Taipei) servicelink 驗證

**日期**: 2026-04-14 | **工具**: test-iridium-e2e | **時長**: 120s / slotInterval=60s / 3 slots
**段配置**: feederlink=off, isl=off, servicelink=on
...
| servicelink 流量 | gwId=0 → 91 UTs，FWD 120kbps + RTN 8.192kbps |
| feeder link 資料 | 有（底層流量同時觸發 feeder trace，分佈與 gw2sat 相同）|
| feeder drop rate | 0.00% |
| OBS 事件 | sat61 service throughput=0（與其他單段場景相同）|
| 動態路由重算 | slot1: 3/66, slot2: 16/66 |

## 觀察

- `sat2ut` 語意為「service link 下行驗證」，底層流量安裝呼叫 `InstallGwUtSegmentTrafficBase`，feeder trace 同樣有資料（sat1/sat15 分佈與 gw2sat/sat2gw 一致）
- 三種單段模式（gw2sat / sat2gw / sat2ut）feeder obs 分佈相同，因為共用 beamId=1 場景（91 UTs）與 gwId=0 接入
...
./ns3 run "test-iridium-e2e \
  ...
  --elevMinDeg=5 \
  --enableServicelink=1 \
  --obsLogPath=Logs/0414_sat2ut_link_obs.csv \
  --obsInterval=10 \
  --obsDropAlertPct=50" | tee "Topology & ISL Routing/Outputs/E2E-PathType/sat2ut_ut0_120s.log"
```
```
...
[OBS] log opened: Logs/0414_sat2ut_link_obs.csv
...
[CHKPT] 120.0s | ApplyRoutingTable: slot=2 t=120.0s | apply=3ms recompute=3ms recomputedSrc=16

=== E2E Link Observability Final Summary ===
link                    rx_pkts   rx_bytes     drop_pkts  drop_rate(%)  avg_delay(ms)
------------------------------------------------------------------------------------
feeder:sat0             0         0            0          0.00          0.00
feeder:sat1             13082     20415808     0          0.00          32.81
...
service:sat1            491       129956       0          0.00          48.38
...
------------------------------------------------------------------------------------
Log: Logs/0414_sat2ut_link_obs.csv
=============================================
