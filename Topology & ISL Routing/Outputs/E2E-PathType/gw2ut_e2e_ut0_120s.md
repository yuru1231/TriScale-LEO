# gw2ut_e2e — GW0 (TW-Taipei) → UT0 (Taipei, 25.033°N/121.565°E)

**日期**: 2026-04-14 | **工具**: test-iridium-e2e | **時長**: 120s / slotInterval=60s / 3 slots
**段配置**: feederlink=on, isl=on, servicelink=on
...
| 路由（全 3 slots） | entry=SAT15, serving=SAT15（無 ISL 跳，isl_cost=N/A）|
| feeder link 活躍衛星 | sat15（32087pkts, 4.62ms）、sat1（13082pkts, 32.81ms）等 |
| feeder drop rate | 0.00% |
| 動態路由重算 | slot1: 3/66, slot2: 16/66 |

## 路由路徑（GW0 TW-Taipei → UT0 Taipei）

| slot | time(s) | entry | ISL path | serving | isl_cost |
|------|---------|-------|----------|---------|----------|
| 0 | 0 | SAT15 | 無 ISL 跳 | SAT15 | N/A |
| 1 | 60 | SAT15 | 無 ISL 跳 | SAT15 | N/A |
| 2 | 120 | SAT15 | 無 ISL 跳 | SAT15 | N/A |

## 觀察

- GW0 (25.0°N/121.5°E) 與 UT0 (25.033°N/121.565°E) 距離 < 10km，同一顆衛星（SAT15）同時服務兩端，無需 ISL 跳，結果與 baseline gw2ut TW→Taipei 一致
- 流量模式與 gw2sat 相同：底層均呼叫 `InstallGwUtSegmentTrafficBase(gwId=0)`
- OBS 事件 sat61 為 false alarm（gw2ut_e2e 有 service link 但 sat61 本身無覆蓋此區域）
...
./ns3 run "test-iridium-e2e \
  ...
  --enableIsl=1 \
  --enableServicelink=1 \
  --obsLogPath=Logs/0414_gw2ut_e2e_link_obs.csv \
  --obsInterval=10 \
  --obsDropAlertPct=50" | tee "Topology & ISL Routing/Outputs/E2E-PathType/gw2ut_e2e_ut0_120s.log"
```
```
...
[OBS] log opened: Logs/0414_gw2ut_e2e_link_obs.csv
...
[CHKPT] 0s | PrecomputeAllTables: complete | wall=0ms
...
[CHKPT] 120.0s | RecomputeAffectedRoutes: slot=2 recomputed=16/66 wall=0ms
[CHKPT] 120.0s | ApplyRoutingTable: slot=2 t=120.0s | apply=0ms recompute=0ms recomputedSrc=16
...
feeder:sat15            32087     49236982     0          0.00          4.62
...
service:sat15           10721     2909392      0          0.00          1404.23
...
------------------------------------------------------------------------------------
Log: Logs/0414_gw2ut_e2e_link_obs.csv
=============================================
