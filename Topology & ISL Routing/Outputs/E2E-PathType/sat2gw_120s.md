# sat2gw — SAT → GW0 (TW-Taipei) feederlink 下行驗證
...
| feeder link 活躍衛星 | sat15（32087pkts, 4.62ms）、sat1（13082pkts, 32.81ms）等 13 顆 |
| feeder drop rate | 0.00% |
| OBS 事件 | sat61 service throughput=0（預期：sat2gw 無 service link）|
| 動態路由重算 | slot1: 3/66, slot2: 16/66 |

## 觀察

- feeder link 流量分佈與 gw2sat_120s 完全相同：兩者均呼叫 `InstallGwUtSegmentTrafficBase(gwId=0)`，方向語意不同（gw2sat=上行，sat2gw=下行），但底層安裝邏輯相同，觀測數據對稱
- gw2sat 與 sat2gw 差異僅在 pathType 語意標籤，無結構上的差異
...
./ns3 run "test-iridium-e2e \
  ...
  --elevMinDeg=5 \
  --enableFeederlink=1 \
  --obsLogPath=Logs/0414_sat2gw_link_obs.csv \
  --obsInterval=10 \
  --obsDropAlertPct=50" | tee "Topology & ISL Routing/Outputs/E2E-PathType/sat2gw_120s.log"
```
```
[CFG] pathType=sat2gw trafficProfile=none simTime=120 slotInterval=60 numSlots=3 lastSlotTime=120
[ISL_DROP] trace connected: 264 ISL interfaces (132 unique links)
[OBS] log opened: Logs/0414_sat2gw_link_obs.csv
...
[E2E] pathType=sat2gw includesIsl=no segments={feederlink=on, isl=off, servicelink=off} traffic={sharedEdge=on, islBg=off, gw2gwBg=off, gw2gwDirect=off}
...
[CASE] sat2gw | gwId=0 | feederlink_dn only | isl_cost=N/A
...
[CHKPT] 120.0s | RecomputeAffectedRoutes: slot=2 recomputed=16/66 wall=2ms
[CHKPT] 120.0s | ApplyRoutingTable: slot=2 t=120.0s | apply=2ms recompute=2ms recomputedSrc=16
...
------------------------------------------------------------------------------------
Log: Logs/0414_sat2gw_link_obs.csv
=============================================
