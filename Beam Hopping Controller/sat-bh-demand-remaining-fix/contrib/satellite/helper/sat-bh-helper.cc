...
SatBhHelper::OpenTrafficTrace()
{
    ...
    if (writeHeader)
    {
        m_trafficTrace
            ...
            << "served_kbps,remaining_kbps,remaining_pct\n";
                              ...
                              double remainingKbps)
{
    ...
    const double remainingPct =
        (demandKbps > 0.0) ? (remainingKbps / demandKbps * 100.0) : 0.0;
...
SatBhHelper::OnBacklogRequestTrace(std::string record)
{
    ...
    m_latestDemandKbps[beamId] = entry.totalRbdcKbps;
...
SatBhHelper::RunDynamicBstpCycle(Time now)
{
    ...
    else
    {
        ...
        if (plan)
        {
            ...
            EmitDemandServiceTrace(conf, periodStart.GetSeconds());
...
SatBhHelper::InjectFwdDemand()
{
    ...
    for (uint32_t bid = 1; bid <= m_cfg.numBeams; ++bid)
    {
        ...
        m_latestDemandKbps[bid] = demandKbps;
        EmitTrafficTrace("DEMAND",
                         Simulator::Now().GetSeconds(),
                         bid,
                         "OFFERED_FWD",
...
void
SatBhHelper::EmitDemandServiceTrace(const SatDynamicBstpProvider::Conf& conf, double timeSec)
{
    if (!m_trafficTrace.is_open())
    {
        return;
    ...
    std::set<uint32_t> active(conf.activeBeams.begin(), conf.activeBeams.end());
    std::ostringstream activeBeams;
    for (std::size_t i = 0; i < conf.activeBeams.size(); ++i)
    {
        if (i > 0)
        {
            activeBeams << "|";
        }
        activeBeams << conf.activeBeams[i];
    }

    for (uint32_t bid = 1; bid <= m_cfg.numBeams; ++bid)
    {
        double demandKbps = 0.0;
        auto it = m_latestDemandKbps.find(bid);
        if (it != m_latestDemandKbps.end())
        {
            demandKbps = it->second;
        }
