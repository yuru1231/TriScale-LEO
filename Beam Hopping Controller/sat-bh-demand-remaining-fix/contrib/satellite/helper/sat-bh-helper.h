...
struct BhExperimentConfig
{
    ...
    // ── Phase G FWD offered-demand injection ──────────────────────────────
    // FWD CBR traffic is the offered demand that the BH scheduler should react
    // to.  Packet Rx is an output/validation signal, not the scheduler input.
    // When enabled, every T_p each beam receives fwdOfferedDemandKbps as the
    // provider's demand and the value is written as DEMAND/OFFERED_FWD rows.
    bool     enableFwdOfferedDemand{false};         ///< Feed FWD offered load to Dynamic BSTP
    double   fwdOfferedDemandKbps{0.0};              ///< Offered FWD demand per beam [kbps]

    // Optional extra demand for explicitly hotter FWD beams.  This is added on
    // top of fwdOfferedDemandKbps for the listed beam IDs.
    std::vector<uint32_t> bhFwdHotspotBeamIds{};    ///< Beam IDs with extra FWD demand
    double   bhFwdHotspotBoostKbps{0.0};             ///< Extra demand for hotspot beams [kbps]
...
class SatBhHelper : public Object
{
  ...
  private:
    ...
    /// Called every T_p when FWD offered-demand injection is enabled.
    /// Feeds the provider with offered FWD demand, then re-schedules itself.
    void InjectFwdDemand();

    /// Emit per-beam demand accounting for the BHTP cycle just selected.
    /// remaining_kbps reports demand that was not assigned service in this cycle.
    void EmitDemandServiceTrace(const SatDynamicBstpProvider::Conf& conf, double timeSec);
    ...
    /// Open and append to the run-level BH traffic/event trace.
    void OpenTrafficTrace();

    /// Emit a single trace row for demand, plan, or OBC real-toggle events.
    void EmitTrafficTrace(const std::string& recordType,
                          double timeSec,
                          uint32_t beamId,
                          ...
                          bool mapped,
                          bool toggled,
                          double servedKbps = 0.0,
                          double remainingKbps = 0.0);
    ...
    std::map<uint32_t, double>  m_latestDemandKbps; ///< Phase G/F: latest offered demand per beam
    ...
    // Real BH traffic/event trace.  All helpers append to the same run-level file;
    // rows are distinguished by sat_id and record_type.
    std::ofstream m_trafficTrace;
