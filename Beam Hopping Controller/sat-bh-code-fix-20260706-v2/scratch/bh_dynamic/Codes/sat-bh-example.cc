 ...
 *   ./ns3 run "sat-bh-example --configFile=scratch/bh_dynamic/Codes/my-run.txt"
 *
 * Configure phases, scenario, ROI, traffic, RF, and outputs in the config file.
 * Bool values accept 0/1, true/false, yes/no, and on/off.
 *
 * DESIGN PRINCIPLE
 * ----------------
...
static ScenarioKind
InferScenarioKind(const std::string& scenarioFolder)
{
    if (scenarioFolder.find("starlink") != std::string::npos ||
        scenarioFolder.find("1584") != std::string::npos)
    {
        return ScenarioKind::STARLINK25;
    }
    if (scenarioFolder.find("iridium") != std::string::npos)
    {
        return ScenarioKind::IRIDIUM;
    }
    if (scenarioFolder.find("leo-2") != std::string::npos ||
        scenarioFolder.find("leo2sat") != std::string::npos)
    {
        return ScenarioKind::LEO2SAT;
    }

    NS_ABORT_MSG("Unsupported scenario folder: " << scenarioFolder);
    return ScenarioKind::LEO2SAT;
}

static uint32_t
InferNumSats(ScenarioKind kind)
{
    switch (kind)
    {
    case ScenarioKind::STARLINK25:
        return 1584;
    case ScenarioKind::IRIDIUM:
        return 66;
    case ScenarioKind::LEO2SAT:
        return 1;
    }
    NS_ABORT_MSG("Unknown ScenarioKind");
    return 0;
}

static std::string
ScenarioTag(ScenarioKind kind)
{
    switch (kind)
    {
    case ScenarioKind::STARLINK25:
...
static std::string
RemoveInlineComment(const std::string& line)
{
    bool inQuotes = false;
...
static std::string
StripOptionalQuotes(const std::string& value)
{
...
static std::string
GetConfigFilePath(int argc, char* argv[])
{
    std::string configFile{"scratch/bh_dynamic/Codes/sat-bh-example.txt"};
    for (int i = 1; i < argc; ++i)
    {
        const std::string arg{argv[i]};
        const std::string prefix{"--configFile="};
        if (arg.rfind(prefix, 0) == 0)
        {
            configFile = arg.substr(prefix.size());
        }
        else if (arg == "--configFile")
        {
            NS_ABORT_MSG_IF(i + 1 >= argc, "--configFile requires a path");
            configFile = argv[++i];
        }
        else
        {
            NS_ABORT_MSG("sat-bh-example now accepts only --configFile=<path>; unexpected argument: "
                         << arg);
        }
    }
    return configFile;
...
LoadInputConfig(const std::string& configFile)
{
    std::ifstream input(configFile);
    std::string resolvedConfigFile = configFile;
    if (!input.is_open())
    {
        std::filesystem::path fallbackPath(configFile);
        if (fallbackPath.extension() == ".conf")
        {
            fallbackPath.replace_extension(".txt");
            input.open(fallbackPath);
            if (input.is_open())
            {
                resolvedConfigFile = fallbackPath.string();
                NS_LOG_WARN("sat-bh-example config file " << configFile
                            << " not found; using " << resolvedConfigFile);
            }
        }
    }
    NS_ABORT_MSG_IF(!input.is_open(),
                    "Cannot open sat-bh-example config file: " << configFile
                                                               << " (also tried .txt fallback)");
    ...
    while (std::getline(input, line))
    {
        ...
        const std::size_t altSeparator = line.find(':');
        std::size_t split = separator;
        if (split == std::string::npos ||
            (altSeparator != std::string::npos && altSeparator < split))
        {
            split = altSeparator;
        }

        NS_ABORT_MSG_IF(split == std::string::npos,
                        "Invalid config line " << configFile << ":" << lineNo
                                               << " (expected key=value)");
        ...
        const std::string value = StripOptionalQuotes(Trim(line.substr(split + 1)));
        NS_ABORT_MSG_IF(key.empty(), "Empty config key at " << configFile << ":" << lineNo);
        values[key] = value;
    }
    return values;
}
...
static bool
TakeString(std::map<std::string, std::string>& values, const std::string& key, std::string& out)
{
...
template <typename T>
static bool
TakeNumber(std::map<std::string, std::string>& values, const std::string& key, T& out)
{
    auto it = values.find(key);
    if (it == values.end())
    {
    ...
    T parsed{};
    iss >> parsed;
    NS_ABORT_MSG_IF(iss.fail() || !iss.eof(),
...
static bool
TakeBool(std::map<std::string, std::string>& values, const std::string& key, bool& out)
{
    auto it = values.find(key);
    if (it == values.end())
    {
    ...
    {
        NS_ABORT_MSG("Invalid bool value for " << key << ": " << it->second);
    }
    values.erase(it);
    return true;
}

// ── ParseConfig: input file → BhExperimentConfig ──────────────────────────
//
// The command line intentionally accepts only --configFile.  All experiment
// knobs live in one key=value file so repeated runs do not need long CLI lines.
            ...
            std::string& outputDir)
{
    ...
    const std::string configFile = GetConfigFilePath(argc, argv);
    auto values = LoadInputConfig(configFile);

    TakeString(values, "scenario", scenario);
    TakeNumber(values, "simTime", cfg.simTimeSec);
    TakeNumber(values, "numPeriods", cfg.numPeriods);
    TakeNumber(values, "warmUp", cfg.warmUpSec);
    TakeNumber(values, "numBeams", cfg.numBeams);
    TakeNumber(values, "maxActiveBeams", cfg.maxActiveBeams);
    TakeNumber(values, "numHotspot", cfg.numHotspotBeams);
    TakeNumber(values, "satId", cfg.satId);
    TakeNumber(values, "alpha", cfg.alphaDelaySensitivity);
    TakeNumber(values, "kappa", cfg.interferenceKappa);
    TakeNumber(values, "slotMs", cfg.slotDurationMs);
    TakeNumber(values, "periodMs", cfg.bhtpPeriodMs);
    TakeNumber(values, "switchMs", cfg.switchingTimeMs);
    TakeNumber(values, "propMs", cfg.propagationDelayMs);
    TakeBool(values, "enableScheduler", cfg.enableScheduler);
    TakeBool(values, "enableObc", cfg.enableObc);
    TakeBool(values, "enableCacheQueue", cfg.enableCacheQueue);
    TakeBool(values, "enablePrecoder", cfg.enablePrecoder);
    TakeBool(values, "enableResourceManager", cfg.enableResourceManager);
    TakeBool(values, "enableUserAssociation", cfg.enableUserAssociation);

    uint32_t schedulingModeArg = cfg.schedulingMode;
    if (TakeNumber(values, "schedulingMode", schedulingModeArg))
    {
        NS_ABORT_MSG_IF(schedulingModeArg > 255,
                        "schedulingMode must fit in uint8_t: " << schedulingModeArg);
        cfg.schedulingMode = static_cast<uint8_t>(schedulingModeArg);
    }

    TakeNumber(values, "maxReassignment", cfg.maxReassignmentPerFrame);
    TakeBool(values, "enablePowerAllocation", cfg.enablePowerAllocation);
    TakeBool(values, "enablePhaseE", cfg.enablePhaseE);
    TakeBool(values, "enablePhaseF", cfg.enablePhaseF);
    TakeBool(values, "enableDynamicBstp", cfg.enableDynamicBstp);
    TakeNumber(values, "bhDemandWeight", cfg.bhDemandBacklogWeight);
    TakeNumber(values, "bhFairnessWeight", cfg.bhFairnessWeight);
    TakeNumber(values, "bhValiditySF", cfg.bhValiditySuperframes);
    TakeNumber(values, "bhStarvationThr", cfg.bhStarvationThreshold);
    TakeBool(values, "enableFwdOfferedDemand", cfg.enableFwdOfferedDemand);
    TakeNumber(values, "fwdOfferedDemandKbps", cfg.fwdOfferedDemandKbps);
    TakeNumber(values, "totalPowerDbm", cfg.totalPowerBudgetDbm);
    TakeNumber(values, "noisePowerDbw", cfg.noisePowerDbw);
    TakeNumber(values, "powerMaxIter", cfg.powerMaxIterations);
    TakeNumber(values, "powerEps", cfg.powerConvergenceEps);
    TakeNumber(values, "interferenceFactor", cfg.interferenceFactor);
    TakeNumber(values, "roiLat", cfg.roiLat);
    TakeNumber(values, "roiLon", cfg.roiLon);
    TakeNumber(values, "roiRadius", cfg.roiRadiusDeg);
    TakeNumber(values, "minElevDeg", cfg.minElevDeg);
    TakeNumber(values, "altitudeKm", cfg.altitudeKm);
    TakeNumber(values, "beamHalfAngle", cfg.beamHalfAngleDeg);
    TakeNumber(values, "maxHelperSats", cfg.maxHelperSats);
    TakeNumber(values, "satIdStart", cfg.satIdStart);
    TakeString(values, "helperSatList", helperSatList);
    TakeString(values, "metricsFile", cfg.metricsOutputFile);
    TakeString(values, "timePlanFile", cfg.timePlanCsvFile);
    TakeString(values, "trafficTraceFile", cfg.trafficTraceFile);
    TakeString(values, "outputDir", outputDir);

    NS_ABORT_MSG_IF(!values.empty(),
                    "Unknown key in " << configFile << ": " << values.begin()->first);
...
main(int argc, char* argv[])
{
    ...
    std::string scenario{"constellation-starlink-1584-sats"};

    std::string helperSatList;
    std::string outputDir;
    BhExperimentConfig cfg = ParseConfig(argc, argv, scenario, helperSatList, outputDir);
    const std::string scenarioFolder = scenario;
    const ScenarioKind scenarioKind = InferScenarioKind(scenarioFolder);
    const std::string scenarioTag = ScenarioTag(scenarioKind);
    const uint32_t numSats = InferNumSats(scenarioKind);
    if (!outputDir.empty())
    {
        std::error_code ec;
    ...
    if (scenarioKind == ScenarioKind::IRIDIUM)
    {
        ...
        simHelper->LoadScenario(scenarioFolder);
    ...
    else if (scenarioKind == ScenarioKind::STARLINK25)
    {
    ...
    // By default constellation-iridium-next-66-sats uses CRA (ConstantAssignmentProvided=true), which causes
    ...
    if (scenarioKind == ScenarioKind::STARLINK25)
    {
    ...
    // Each run creates a new file, so previous results are never overwritten.
    {
        ...
        std::string runTag = scenarioTag + "_" + ts.str();
    ...
    std::vector<uint32_t> helperSatIds =
        (scenarioKind == ScenarioKind::STARLINK25) ? starlinkBeamSatIds
                                                   : ParseHelperSatList(helperSatList, numSats);
    ...
    if (helperSatIds.empty())
    {
        ...
        if (scenarioKind == ScenarioKind::STARLINK25 && cfg.maxHelperSats > 0)
        {
    ...
    {
        std::cout << "[" << scenarioTag << "] helperSatList: installing "
                  << helperSatIds.size()
                  << " BH helpers satIds="
    ...
    std::cout << "\n[BH Example] Simulation complete.\n"
              ...
              << "  Actual demand: "
              << (scenarioKind == ScenarioKind::STARLINK25 ? actualDemandPath.string()
    ...
    if (scenarioKind == ScenarioKind::STARLINK25 && !g_beamPdr.empty())
    {
