...
SatUtMac::ScheduleTimeSlots(Ptr<SatTbtpMessage> tbtp)
{
    ...
    // A broadcast TBTP may arrive more than once through parallel control paths.
    // If a duplicate reaches the UT after its superframe transmission time, it
    // is stale and must be ignored. Aborting the whole simulation here makes
    // sparse constellation/beam-hopping scenarios fail even though the first
    // copy was received and scheduled correctly.
    if (txTime < Simulator::Now())
    {
        NS_LOG_WARN("UT: " << m_nodeInfo->GetMacAddress() << " discarding stale TBTP "
                            << tbtp->GetSuperframeCounter() << " (txTime="
                            << txTime.GetSeconds() << "s, now="
                            << Simulator::Now().GetSeconds() << "s)");
        return;
    }

    // Add only valid future TBTPs to the scheduling container.
