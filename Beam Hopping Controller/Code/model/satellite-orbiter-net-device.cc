...
SatOrbiterNetDevice::ReceiveFromIsl(Ptr<Packet> packet, Mac48Address destination)
{
    ...
    auto gwIt = m_gwConnected.find(destination);
    if (gwIt != m_gwConnected.end())
    {
        SatUplinkInfoTag satUplinkInfoTag;
        if (!packet->PeekPacketTag(satUplinkInfoTag))
        {
        ...
        DynamicCast<SatOrbiterFeederMac>(m_feederMac[gwIt->second])->EnquePacket(packet);
    }
    else
    {
        auto utIt = m_utConnected.find(destination);
        if (utIt != m_utConnected.end() || destination.IsBroadcast())
        {
            SatUplinkInfoTag satUplinkInfoTag;
            if (!packet->PeekPacketTag(satUplinkInfoTag))
            {
            ...
            uint32_t beamId = destination.IsBroadcast() ? satUplinkInfoTag.GetBeamId() : utIt->second;
            DynamicCast<SatOrbiterUserMac>(m_userMac[beamId])->EnquePacket(packet);
        }
        if ((m_utConnected.count(destination) == 0 || destination.IsBroadcast()) &&
            m_islNetDevices.size() > 0)
        {
