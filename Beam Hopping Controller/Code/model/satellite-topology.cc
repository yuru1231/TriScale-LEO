...
SatTopology::GetGwAddressInUt(uint32_t utId)
{
    ...
    uint32_t utSatId = GetUtSatId(utNode);
    uint32_t utBeamId = GetUtBeamId(utNode);
    Ptr<Node> gwNode = GetGwFromBeam(utBeamId);

    // GW layers are indexed by the user-link (satellite, beam) pair in
    // AddGwLayers(). Recomputing the GW's closest satellite here only works
    // accidentally when every satellite has an identical beam set, and fails
    // for sparse constellation snapshots such as one selected satellite with
    // 25 enabled beams.
    Ptr<SatGwMac> gwMac = GetGwMac(gwNode, utSatId, utBeamId);
...
SatTopology::PrintTopology(std::ostream& os) const
{
    ...
    {
        orbiter = *itOrbiter;
        auto layersIt = m_orbiterLayers.find(orbiter);
        if (layersIt == m_orbiterLayers.end())
        {
            continue; // Sparse constellation scenario: no enabled beams on this orbiter.
        }
        const OrbiterLayers_s layers = layersIt->second;
    ...
    for (NodeContainer::Iterator itUt = m_uts.Begin(); itUt != m_uts.End(); itUt++)
    {
        ...
        os << "    " << utLayers.m_mac->GetAddress() << ", sat: " << utSatId
           << ", beam: " << utBeamId;
        os << ". Linked to GW "
           << gwLayers.m_mac.at(std::make_pair(utSatId, utBeamId))->GetAddress() << std::endl;
    }
