...
class SatHelper : public Object
{
  public:
    ...
    /**
     * \brief Override the gateway serving a configured beam.
     */
    void SetBeamGatewayId(uint32_t beamId, uint32_t gwId);
  ...
  private:
    ...
    /**
     * Install inter-satellite links from the scenario. Sparse fixed snapshots
     * may disable these when only one orbiter has user/feeder layers.
     */
    bool m_islsEnabled;
