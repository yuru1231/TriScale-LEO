...
#include <cmath>
#include <filesystem>
#include <iomanip>   // std::fixed, std::setprecision — for sat-bh-traffic.tr output
#include <set>
#include <fstream>
#include <iostream>
...
SatBhHelper::Install()
{
    ...
    // Open BH traffic trace file (PLAN/EVENT/DEMAND rows)
    if (!m_cfg.trafficTraceFile.empty())
    {
        m_trafficOfs.open(m_cfg.trafficTraceFile);
        NS_ASSERT_MSG(m_trafficOfs.is_open(),
                      "SatBhHelper: cannot open traffic trace file: "
                          << m_cfg.trafficTraceFile);
        m_trafficOfs << "record_type,time_s,sat_id,beam_id,event,"
                        "plan_id,slot_idx,demand_kbps,duration_ms,active_beams,mapped,toggled\n";
        NS_LOG_INFO("SatBhHelper: traffic trace opened: " << m_cfg.trafficTraceFile);
    }
...
SatBhHelper::SetupObc()
{
    ...
    m_obc->SetBeamActivateCallback(
        [this](uint32_t satId, uint32_t beamId, Time usableDur) {
            ...
            // Write EVENT ACTIVATE row to traffic trace.
            if (m_trafficOfs.is_open())
            {
                uint32_t planId = m_lastDynamicPlan ? m_lastDynamicPlan->GetPlanId() : m_dynamicPlanId;
                m_trafficOfs << std::fixed << std::setprecision(6)
                             << "EVENT," << Simulator::Now().GetSeconds()
                             << "," << satId << "," << beamId << ",ACTIVATE,"
                             << planId << ",-1,"
                             << std::setprecision(3)
                             << 0.0 << "," << static_cast<double>(usableDur.GetMilliSeconds()) << ",,"
                             << mappedVal << "," << mappedVal << "\n";
            }
        });
    ...
    m_obc->SetBeamDeactivateCallback(
        [this](uint32_t satId, uint32_t beamId, Time usedDur) {
            ...
            // Write EVENT DEACTIVATE row to traffic trace.
            if (m_trafficOfs.is_open())
            {
                uint32_t planId = m_lastDynamicPlan ? m_lastDynamicPlan->GetPlanId() : m_dynamicPlanId;
                m_trafficOfs << std::fixed << std::setprecision(6)
                             << "EVENT," << Simulator::Now().GetSeconds()
                             << "," << satId << "," << beamId << ",DEACTIVATE,"
                             << planId << ",-1,"
                             << std::setprecision(3)
                             << 0.0 << "," << static_cast<double>(usedDur.GetMilliSeconds()) << ",,"
                             << mappedVal << "," << mappedVal << "\n";
            }
        });
}
...
SatBhHelper::OnBacklogRequestTrace(std::string record)
{
    ...
    // Write DEMAND row to traffic trace.
    // One row per BacklogRequestsTrace callback (one per Return Channel per beam per DAMA cycle).
    // demand_kbps shows the running accumulated total across RCs within this DAMA cycle.
    if (m_trafficOfs.is_open())
    {
        uint32_t planId = m_lastDynamicPlan ? m_lastDynamicPlan->GetPlanId() : m_dynamicPlanId;
        m_trafficOfs << std::fixed << std::setprecision(6)
                     << "DEMAND," << timeS
                     << "," << m_cfg.satId << "," << beamId << ",RBDC,"
                     << planId << ",-1,"
                     << std::setprecision(3)
                     << entry.totalRbdcKbps << ",0.000,,0,0\n";
    }
...
SatBhHelper::ConfToTimePlan(const SatDynamicBstpProvider::Conf& conf, Time periodStart)
{
    ...
    for (uint32_t s = 0; s < M; s++)
    {
        ...
        // Write PLAN row: one per slot, records the beam assignment for this slot.
        // time_s = absolute slot start (periodStart + slot offset); beam_id=0 (slot-level row).
        if (m_trafficOfs.is_open())
        {
            Time absSlotStart = periodStart + slot.startTime;
            std::string beamStr;
            for (size_t ki = 0; ki < slot.beamIds.size(); ++ki)
            {
                if (ki) beamStr += "|";
                beamStr += std::to_string(slot.beamIds[ki]);
            }
            m_trafficOfs << std::fixed << std::setprecision(6)
                         << "PLAN," << absSlotStart.GetSeconds()
                         << "," << m_cfg.satId << ",0,SLOT,"
                         << m_dynamicPlanId << "," << s << ","
                         << std::setprecision(3)
                         << 0.0 << "," << static_cast<double>(slot.duration.GetMilliSeconds()) << ","
                         << beamStr << ",0,0\n";
        }
    }
