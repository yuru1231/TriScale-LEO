...
IslRoutingManager::ApplyRoutingTable(uint32_t slotIndex)
{
    ...
    InstallSatIpRoutesForGwPairs(slotIndex);
...
IslRoutingManager::InstallSatIpRoutesForGwPairs(uint32_t slotIndex)
{
    ...
    for (const auto& kv : m_gwRoutes[slotIndex])
    {
        ...
        // Find the destination GW feeder interface attached to the selected route exit
        // satellite.  Do not use the first routable GW interface; constellation GW nodes
        // may have many SatNetDevices.
        ...
        {
            Ptr<SatNetDevice> satDev = DynamicCast<SatNetDevice>(gwNode->GetDevice(d));
            if (!satDev)
            {
                continue;
            }

            Ptr<SatGwMac> gwMac = DynamicCast<SatGwMac>(satDev->GetMac());
            if (!gwMac || gwMac->GetSatId() != route.exitSatId)
            {
                continue;
            }

            int32_t ifIndex = gwIpv4->GetInterfaceForDevice(satDev);
            if (ifIndex < 0)
            {
                continue;
            }

            uint32_t i = static_cast<uint32_t>(ifIndex);
