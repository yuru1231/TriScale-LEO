...
SimulationHelper::SimulationHelper()
    : m_satHelper(nullptr),
      ...
      m_enabledBeamsStr(""),
      m_enabledBeams(),
      m_constellationBeamUserInfo(),
      m_constellationTimeOffset(Seconds(0)),
      m_customConstellationGatewayId(0),
      m_outputPath(""),
      m_utCount(),
      m_utUserCount(0),
      m_utMobileUserCount(0),
...
SimulationHelper::DoDispose(void)
{
    ...
    m_constellationBeamUserInfo.clear();
}
                                    ...
                                    const std::string& mobileUtsFolder)
{
    ...
    if (!m_constellationTimeOffset.IsZero() && m_satHelper->IsSatConstellationEnabled())
    {
        NodeContainer orbiters = Singleton<SatTopology>::Get()->GetOrbiterNodes();
        for (uint32_t i = 0; i < orbiters.GetN(); ++i)
        {
            Ptr<SatSGP4MobilityModel> mobility =
                orbiters.Get(i)->GetObject<SatSGP4MobilityModel>();
            NS_ASSERT_MSG(mobility, "Constellation orbiter " << i << " has no SGP4 mobility model");
            mobility->SetStartTime(mobility->GetStartTime() + m_constellationTimeOffset);
        }
    }
    ...
    if (m_satHelper->IsSatConstellationEnabled())
    {
        ...
        if (!m_constellationBeamUserInfo.empty())
        {
            NS_ABORT_MSG_IF(scenario != SatHelper::NONE,
                            "Custom constellation beam information requires SatHelper::NONE");
            beamInfo = m_constellationBeamUserInfo;
            if (m_customConstellationGatewayId != 0)
            {
                std::set<uint32_t> configuredBeams;
                for (const auto& [satBeam, info] : beamInfo)
                {
                    (void)info;
                    if (configuredBeams.insert(satBeam.second).second)
                    {
                        m_satHelper->SetBeamGatewayId(satBeam.second,
                                                      m_customConstellationGatewayId);
                    }
                }
            }
            antennaGainPatterns->SetEnabledBeams(beamInfo);
        ...
        else
        {
            ...
            case SatHelper::FULL: {
                for (uint32_t satId = 0;
                     satId < Singleton<SatTopology>::Get()->GetNOrbiterNodes();
                     satId++)
                {
                    // Set beamInfo to indicate enabled beams
                    for (uint32_t i = 1; i <= m_satHelper->GetBeamCount(); i++)
...
void
SimulationHelper::SetConstellationBeamUserInfo(const SatHelper::BeamUserInfoMap_t& beamInfo)
{
    NS_LOG_FUNCTION(this << beamInfo.size());
    m_constellationBeamUserInfo = beamInfo;
}

void
SimulationHelper::SetConstellationTimeOffset(Time offset)
{
    NS_LOG_FUNCTION(this << offset);
    m_constellationTimeOffset = offset;
}

void
SimulationHelper::SetCustomConstellationGatewayId(uint32_t gwId)
{
    NS_LOG_FUNCTION(this << gwId);
    m_customConstellationGatewayId = gwId;
}
