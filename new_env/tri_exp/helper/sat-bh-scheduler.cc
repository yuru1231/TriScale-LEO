...
SatBhScheduler::ComputeInterferenceFactor(uint32_t beamI, uint32_t beamJ) const
{
    ...
    // Beam ground radius derived from altitude and beam half-angle [km].
    // Replaces the old hardcoded 20.0 km (implicit Starlink 550 km / 2.0° assumption).
    const double pi         = std::acos(-1.0);
    const double rMiddle    = m_altitudeKm * std::tan(m_beamHalfAngleDeg * pi / 180.0);
    const double firstLayer = 1.5 * (rMiddle + rMiddle);
