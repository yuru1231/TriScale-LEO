    ...
    static TypeId tid =
        TypeId("ns3::SatHelper")
            ...
            .AddAttribute("HandoversEnabled",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("IslsEnabled",
                          "Install inter-satellite links from the constellation scenario.",
                          BooleanValue(true),
                          MakeBooleanAccessor(&SatHelper::m_islsEnabled),
                          MakeBooleanChecker())
                                     ...
                                     std::vector<std::pair<uint32_t, uint32_t>>& isls)
{
    ...
    if (m_islsEnabled &&
        Singleton<SatEnvVariables>::Get()->IsValidFile(m_scenarioPath + "/positions/isls.txt"))
    {
...
void
SatHelper::SetBeamGatewayId(uint32_t beamId, uint32_t gwId)
{
    NS_LOG_FUNCTION(this << beamId << gwId);
    m_satConf->SetBeamGatewayId(beamId, gwId);
}
