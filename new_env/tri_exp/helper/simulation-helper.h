...
class SimulationHelper : public Object
{
  public:
    ...
    /**
     * \brief Override the automatically generated constellation beam map.
     *
     * Each key is (satellite ID, beam ID). Positions stored in SatBeamUserInfo
     * are used when creating the UTs, so callers can build a fixed constellation
     * snapshot without enabling the same beam set on every satellite.
     *
     * \param beamInfo Exact constellation beams and UTs to create.
     */
    void SetConstellationBeamUserInfo(const SatHelper::BeamUserInfoMap_t& beamInfo);

    /**
     * \brief Shift the constellation epoch used at simulation time zero.
     *
     * This is useful for fixed snapshots selected at a known offset from the
     * scenario start date, while retaining the original TLE data.
     *
     * \param offset Time added to every SGP4 mobility model start time.
     */
    void SetConstellationTimeOffset(Time offset);

    /**
     * \brief Force every custom constellation beam to use one gateway.
     * \param gwId Gateway ID (1-based); zero keeps scenario defaults.
     */
    void SetCustomConstellationGatewayId(uint32_t gwId);
  ...
  private:
    ...
    SatHelper::BeamUserInfoMap_t m_constellationBeamUserInfo;
    Time m_constellationTimeOffset;
    uint32_t m_customConstellationGatewayId;
