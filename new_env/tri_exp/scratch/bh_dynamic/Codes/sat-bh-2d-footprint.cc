...
struct FootprintConfig
{
    ...
    double   simTimeSec       = 0.0;           // 模擬時長（秒），0 = 只驗證幾何
...
ParseConfig(int argc, char* argv[])
{
    ...
    // Auto-derive rFootprint_m when not explicitly set.
    // The footprint is the inscribed-circle radius of the Nx×Ny beam grid:
    //   cellW = 2 × altitudeKm × tan(beamHalfAngle)
    //   L     = max(Nx,Ny) × cellW
    //   r     = L / sqrt(2)   [inscribed circle of the square]
    if (cfg.rFootprint_m <= 0.0)
    {
        const double altitudeM = cfg.altitudeKm * 1000.0;
        const double halfAngleRad = cfg.beamHalfAngleDeg * M_PI / 180.0;
        const double cellWidthM = 2.0 * altitudeM * std::tan(halfAngleRad);
        const double gridSideM = std::max(cfg.Nx, cfg.Ny) * cellWidthM;
        cfg.rFootprint_m = gridSideM / std::sqrt(2.0);
...
main(int argc, char* argv[])
{
    ...
    for (const auto& c : cells)
    {
        ...
        SatBeamUserInfo info(1, 1);
        info.SetPositions({{GeoCoordinate(c.lat_deg, c.lon_deg, 0.0), 0}});
        beamInfos[{cfg.satId, c.beamId}] = info;
    }

    simHelper->SetConstellationBeamUserInfo(beamInfos);
